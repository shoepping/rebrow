...
def server_db(host, port, db):
    ...
    """
    s = time.time()
    r = redis.StrictRedis(host=host, port=port, ssl=True, db=0, password=session['password'])
    info = r.info("all")
    dbsize = r.dbsize()
...
def keys(host, port, db):
    ...
    """
    s = time.time()
    r = redis.StrictRedis(host=host, port=port, ssl=True, db=db, password=session['password'])
    if request.method == "POST":
